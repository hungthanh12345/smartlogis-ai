...
def get_current_user_optional(request: Request, token: Optional[str] = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    # 1. Kiểm tra session ID (sid) xem phiên có bị vô hiệu hóa hoặc bị đá ra bởi phiên đăng nhập mới không
    sid = payload.get("sid")
    if sid and not session_manager.is_session_valid(username, sid):
        return None

    # 2. Kiểm tra header X-Session-User (Tab Isolation & Chống mượn quyền chéo tab)
    session_user_header = request.headers.get("X-Session-User")
    if session_user_header and session_user_header.strip() and session_user_header.strip() != username:
        return None

    user = db.query(NguoiDung).filter(NguoiDung.TenDangNhap == username).first()
    return user

def get_current_active_user(request: Request, user = Depends(get_current_user_optional)):
    """Bắt buộc người dùng đã xác thực, nếu không ném lỗi HTTP 401."""
    if not user:
        session_user_header = request.headers.get("X-Session-User")
        detail = "Phiên làm việc hết hạn hoặc chưa đăng nhập."
        if session_user_header:
            detail = f"Xung đột phiên làm việc: Tab này thuộc tài khoản '{session_user_header}', không khớp với phiên hệ thống hiện tại. Vui lòng đăng nhập lại."
        raise HTTPException(
